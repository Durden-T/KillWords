...
#define WIN32_LEAN_AND_MEAN     //库需求定义
...
命令		客户端发送				服务端返回
登录		name key				count exp level type/wrongKey/hasLogined/EMPTY
存在		name					EXIST/EMPTY
注册		name key type			EMPTY
取词		easy/medium/hard		word/EMPTY
出题		word					EMPTY/EXIST
查询		name					count exp level type/EMPTY
排行		a/b/c(数目/经验/等级)	player/wordMaker	name count exp level … or EMPTY
更新		name count exp level	EMPTY
登出		name					EMPTY
*/
...
+-------+------------+------+-----+---------+-------+
| name  | char(128)  | NO   | PRI | NULL    |       |
| count | tinyint(4) | NO   |     | 0       |       |
| exp   | tinyint(4) | NO   |     | 0       |       |
| level | tinyint(4) | NO   |     | 0       |       |
| key   | char(128)  | NO   |     | NULL    |       |
| type  | char(4)    | NO   |     | NULL    |       |
+-------+------------+------+-----+---------+-------+
